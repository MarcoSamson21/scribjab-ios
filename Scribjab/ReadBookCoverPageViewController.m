...
@interface ReadBookCoverPageViewController () <AVAudioPlayerDelegate, LoginViewControllerDelegate, FlagBookViewControllerDelegate, NSURLConnectionDelegate>
{
    User * _loggedInUser;
    
    NSString * _title1AudioPath;
    NSString * _title2AudioPath;
...
@implementation ReadBookCoverPageViewController
...
- (void)viewDidLoad
{
    ...
    NSString * screenName = [NSString stringWithFormat:@"%@ (%@ class)", @"Read Book (Title Page) Screen", [self class]];
    [tracker set:kGAIScreenName value:screenName];
    
    // Book Title Dimension
    NSString * bookTitleDimentionValue = [NSString stringWithFormat:@"%@ | %@", _book.title1, _book.title2];
    [tracker set:[GAIFields customDimensionForIndex:1] value:bookTitleDimentionValue];
    
    // Book ID
    NSString * bookIdDimentionValue = _book.remoteId.stringValue;
...
- (void) initializeViewController
{
    ...
    self.title1.layer.cornerRadius = ROUNDED_CORNER_RADIUS;
    self.title1.layer.masksToBounds = YES;
    self.title2.layer.cornerRadius = ROUNDED_CORNER_RADIUS;
    self.title2.layer.masksToBounds = YES;
    
    // load book data
    self.title1.text = [@" " stringByAppendingString:_book.title1];
    self.title2.text = [@" " stringByAppendingString:_book.title2];
    self.text1.text = _book.description1;
    self.text2.text = _book.description2;
    self.author.text = [self.author.text stringByAppendingString:_book.author.userName];
    
    // Add image
    UIImage * image = [UIImage imageWithContentsOfFile:[BookManager getBookItemAbsPath:_book fileName:BOOK_IMAGE_FILENAME]];
    self.image.image = image;
    self.image.backgroundColor = [UIColor colorWithHexString:_book.backgroundColorCode];
    
    // Check if sound exists
    _title1AudioPath = [BookManager getBookItemAbsPath:_book fileName:BOOK_TITLE_1_AUDIO_FILENAME_MP3];
    _title2AudioPath = [BookManager getBookItemAbsPath:_book fileName:BOOK_TITLE_2_AUDIO_FILENAME_MP3];
    _text1AudioPath  = [BookManager getBookItemAbsPath:_book fileName:BOOK_DESC_1_AUDIO_FILENAME_MP3];
    _text2AudioPath  = [BookManager getBookItemAbsPath:_book fileName:BOOK_DESC_2_AUDIO_FILENAME_MP3];
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:_title1AudioPath])
    {
        _title1AudioPath = nil;
        [self.playTitle1Button setHidden:YES];
    }
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:_title2AudioPath])
    {
        _title2AudioPath = nil;
        [self.playTitle2Button setHidden:YES];
    }
    ...
    if (_title1AudioPath == nil && _title2AudioPath == nil && _text1AudioPath == nil && _text2AudioPath == nil)
    {
    ...
    //    self.menuExpandButton.frame = CGRectMake(self.menuExpandButton.frame.origin.x + self.menuBarView.frame.size.width, self.menuExpandButton.frame.origin.y, self.menuExpandButton.frame.size.width, self.menuExpandButton.frame.size.height);
        [self.menuBarView setHidden:YES];
        _menuHidden = YES;
        [self.menuExpandButton setSelected:NO];
    }
    else
    {
        [self.menuExpandButton setSelected:YES];
    }
    // process user data
    [self processUserInformation];
}
...
{
    if (_title1AudioPath == nil) return;
    if (_isReadingAudio) return;
    
    [self playAudioFile:_title1AudioPath initiatedBy:sender];
}
// ======================================================================================================================================

- (IBAction)playTitle2:(id)sender
{
    if (_title2AudioPath == nil) return;
    if (_isReadingAudio) return;
    
    [self playAudioFile:_title2AudioPath initiatedBy:sender];
}
// ======================================================================================================================================
...
- (IBAction)menuButtonClick:(id)sender
{
    ...
    [UIView animateWithDuration:0.5f delay:0.0f options:UIViewAnimationCurveEaseInOut
     ...
     ^{
        ...
        self.menuBarView.frame = CGRectMake(self.menuBarView.frame.origin.x + multiplier * self.menuBarView.frame.size.width, self.menuBarView.frame.origin.y, self.menuBarView.frame.size.width, self.menuBarView.frame.size.height);
       // self.menuExpandButton.frame = CGRectMake(self.menuExpandButton.frame.origin.x + multiplier * self.menuBarView.frame.size.width, self.menuExpandButton.frame.origin.y, self.menuExpandButton.frame.size.width, self.menuExpandButton.frame.size.height);
    ...
    } completion:^(BOOL finished)
    {
       // if (_menuHidden)
     //   {
            //self.menuExpandButton.titleLabel.text = @">";
    //    }
    //    else
...
- (IBAction)readToMe:(id)sender
{
    ...
    if (_title1AudioPath != nil)
        [_readToMeFileList addObject:_title1AudioPath];
    if (_title2AudioPath != nil)
        [_readToMeFileList addObject:_title2AudioPath];
...
{    
    ...
    if ([_loggedInUser.likedBooks containsObject:_book])
    {
        url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@", URL_SERVER_BASE_URL_AUTH, URL_UNLIKE_BOOK, _book.remoteId]];
        connID = CONNECTION_UNLIKE_BOOK;
        [self.likeButton setSelected:NO];
    }
    else
    {
        url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@", URL_SERVER_BASE_URL_AUTH, URL_LIKE_BOOK, _book.remoteId]];
...
- (IBAction)facebookShare:(id)sender
{
    ...
    NSString * urlStr = [NSString stringWithFormat:URL_BOOK_GET_WEB_URL, _book.remoteId];
...
{    
    ...
    if ([_loggedInUser.flaggedBooks containsObject:_book])
        [self.flagBookButton setEnabled:NO];
    if ([_loggedInUser.likedBooks containsObject:_book])
        [self.likeButton setSelected:YES];
    else
        [self.likeButton setSelected:NO];
...
- (void) processLikeURLResponce
{
    ...
    {
        _book.likedBy = _loggedInUser;
        _book.likeCount = [NSNumber numberWithInt:[_book.likeCount intValue] + count];
    }
    else
    {
        [_loggedInUser removeLikedBooksObject:_book];
        _book.likeCount = [NSNumber numberWithInt:[_book.likeCount intValue] - count];
    }
